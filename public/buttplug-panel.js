...
// Built-in haptic patterns: each step has intensity (0-1) and durationMs
const BP_PATTERNS = {
...
class ButtplugPanel {
  ...
  _injectPatternPicker() {
    ...
    `;
    this._panelBody.insertBefore(wrap, this._deviceList);
  ...
  _renderDevice(device) {
    ...
    const canVibrate   = numVibrators > 0;
    ...
    document.getElementById(`bp-stop-${device.index}`).addEventListener('click', async () => {
  ...
  async _allStop() {
    ...
    this._devices.forEach((d, idx) => this._resetDeviceSliders(idx, d.vibrateAttributes.length));
  }

  // ── Public API ────────────────────────────────────────────────────────────────

  /** One-shot pulse – called from chat.js on @mention notification. */
